    ...
    const std::vector<T> &x, std::vector<T> &fERBs, T nyquist, int w, int w2)
{
	...
	std::vector<float> fi(w);
	std::vector<std::complex<float>> fo(w);
    
    kiss_fftr_cfg plan = kiss_fftr_alloc(w, false, 0, 0);
