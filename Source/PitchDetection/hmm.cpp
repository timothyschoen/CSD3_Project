...
template <typename T>
std::vector<size_t>
pitch_detail::bin_pitches(const std::vector<std::pair<T, T>> pitch_candidates)
{
    ...
    const std::vector<std::pair<T, T>> pitch_candidates)
{
	...
	for (auto obs_to_add : pitch_detail::bin_pitches(pitch_candidates)) {
